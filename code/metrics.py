...
def convert_to_one_hot(y_pred, num_classes=4):
    """Converts predicted logits to one-hot format."""
    y_pred = torch.argmax(y_pred, dim=1)
    return F.one_hot(y_pred, num_classes=num_classes).permute(0, 3, 1, 2).float()

def mean_dice_score(y_true, y_pred):
    """Computes the mean Dice coefficient across CSF, GM, and WM classes."""
    y_pred = convert_to_one_hot(y_pred)
    y_true = y_true.float()
...
def weighted_categorical_crossentropy(y_true, y_pred, weights=[1, 10, 3, 3]):
    ...
    loss = -torch.sum(loss, dim=1)  # Summing over classes
